...
func set_callbacks(aabb_cb: FuncRef):
	_aabb_func = aabb_cb
...
func _on_terrain_transform_changed(gt: Transform):
	...
	for k in _mmis:
		var mmi = _mmis[k]
		var aabb = _aabb_func.call_func(Vector3(k.x, 0, k.y) * CHUNK_SIZE)
...
func update(terrain, local_viewer_pos: Vector3, x_range: Array, z_range: Array):
	...
	if DEBUG:
		...
		for cz in z_range:
			for cx in x_range:
				_add_debug_cube(terrain, _aabb_func.call_func(Vector3(cx, 0, cz) * CHUNK_SIZE))
	...
	for cz in z_range:
		for cx in x_range:
			...
			var aabb = _aabb_func.call_func(Vector3(cx, 0, cz) * CHUNK_SIZE)
